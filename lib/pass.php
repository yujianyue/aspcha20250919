    ...
    <body>
        ...
        <script>
            ...
            function changePassword() {
                ...
                ajaxRequest('user.php?do=pass&act=change', Object.fromEntries(formData), function(response) {
                    hideLoading();
...
function changePassword() {
    ...
    // 获取当前用户
    $user = $db->fetchRow("SELECT * FROM `{$table_prefix}_user` WHERE id = {$_SESSION['user_id']}");
    if (!$user) {
        error('用户不存在');
    }
    ...
    $db->update("{$table_prefix}_user", ['password' => $hashed_password], "id = {$_SESSION['user_id']}");
    
    // 记录日志
    writeLog($_SESSION['user_phone'], $_SESSION['user_domain'], getClientIP(), 'change_password', '修改密码');
